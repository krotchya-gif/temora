...
export function EventForm({ mode, eventId, initial, tier, appearance = true }: EventFormProps) {
  ...
  return <form onSubmit={handleSubmit} className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_300px] lg:items-start">
    <div className="space-y-8">
      ...
      {appearance ? <>
      <section className="space-y-5 border-t border-border pt-8" aria-labelledby="event-frame-heading">
        <SectionHeader id="event-frame-heading" eyebrow="02 · Frame & Kartu QR" title="Bingkai foto & kartu meja" description="Upload frame transparan 3:4 — bingkai ini tampil di kamera tamu dan ikut tersimpan di foto hasil. Template kartu QR meja diatur di bawahnya." />
